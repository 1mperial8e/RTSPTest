...
- (CGImageRef)currentCGImage
{
    if (!pFrame->data[0]) return nil;
    [self convertFrameToRGB];
    return [self CGImageFromAVPicture:picture width:outputWidth height:outputHeight];
}
...
- (id)initWithVideo:(NSString *)moviePath usesTcp:(BOOL)usesTcp
{
    ...
    if (avformat_open_input(&pFormatCtx, [moviePath UTF8String], NULL, &opts) !=0 ) {
        ...
        [[[UIAlertView alloc] initWithTitle:@"Error" message:@"Couldn't open file" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil] show];
...
- (CGImageRef)CGImageFromAVPicture:(AVPicture)pict width:(int)width height:(int)height
{
    CGBitmapInfo bitmapInfo = kCGBitmapByteOrderDefault;
    CFDataRef data = CFDataCreateWithBytesNoCopy(kCFAllocatorDefault, pict.data[0], pict.linesize[0]*height,kCFAllocatorNull);
    CGDataProviderRef provider = CGDataProviderCreateWithCFData(data);
    ...
    CGDataProviderRef provider1 = CGDataProviderCreateWithCFData((CFDataRef)UIImageJPEGRepresentation([UIImage imageWithCGImage:cgImage], 1.0));
    ...
    CGImageRef cgim = CGImageCreateWithJPEGDataProvider(provider1, NULL, YES, kCGRenderingIntentDefault);
